...
# -*- coding: utf-8 -*-
"""
实时情绪PAD计算系统（EEG+PPG+GSR融合）
使用 asrpy 进行实时伪影去除（30秒校准）
"""
...
from scipy.signal import find_peaks, butter, filtfilt, savgol_filter, welch
                ...
                while self.running:
                    try:
                        ...
                        if len(numbers) >= 3:
                            ...
                            timestamp = time.time()
                            self.queue.put((timestamp, filter_ppg, gsr_avg))
...
class PADCubeVisualizer:
    def __init__(self, history_length=120):
        ...
        self.plotter.add_mesh(cube, color='lightblue', opacity=0.1, smooth_shading=True)
        self.plotter.add_mesh(cube, style='wireframe', color='gray', line_width=1, opacity=0.6)

        origin = pv.Sphere(center=(0,0,0), radius=0.02)
        self.plotter.add_mesh(origin, color='gold', smooth_shading=True)
        ...
        for name, px, ay, dz, color in base_emotions:
            ...
            self.plotter.add_mesh(sphere, color=color, opacity=1.0, smooth_shading=True)
            self.plotter.add_point_labels([(px+0.1, ay+0.1, dz+0.1)], [name],
                                          text_color='black', font_size=12,
                                          point_size=0, show_points=False)

        self.history_cloud = pv.PolyData()
        self.history_actor = self.plotter.add_mesh(
            self.history_cloud,
        ...
        self.emotion_mesh = pv.Sphere(center=(0,0,0), radius=0.02)
        self.emotion_actor = self.plotter.add_mesh(self.emotion_mesh, color='green',
                                                   smooth_shading=True, specular=0.8)

        label_offset = 1.4
        self.plotter.add_point_labels([(label_offset, 0, 0)], ["Pleasure"],
                                       text_color='red', font_size=20, point_size=0, show_points=False)
        self.plotter.add_point_labels([(0, label_offset, 0)], ["Arousal"],
                                       text_color='green', font_size=20, point_size=0, show_points=False)
        self.plotter.add_point_labels([(0, 0, label_offset)], ["Dominance"],
                                       text_color='blue', font_size=20, point_size=0, show_points=False)

        self.title_actor = self.plotter.add_text("P: 0.00  A: 0.00  D: 0.00",
    ...
    def update(self, pad):
        x, y, z = pad['P'], pad['A'], pad['D']
        self.emotion_mesh.points = pv.Sphere(center=(x,y,z), radius=0.02).points
        self.emotion_actor.mapper.Modified()

        self.history_points.append((x, y, z))
        if len(self.history_points) > self.history_length:
            self.history_points.pop(0)
        if self.history_points:
            points = np.array(self.history_points)
            cloud = pv.PolyData(points)
            time_vals = np.linspace(0, 1, len(points))
            cloud.point_data['time'] = time_vals
            self.history_actor.mapper.SetInputData(cloud)
        else:
            self.history_actor.mapper.SetInputData(pv.PolyData())
        self.history_actor.mapper.Modified()

        self.plotter.remove_actor(self.title_actor)
        self.title_actor = self.plotter.add_text(
            f"P: {x:.2f}  A: {y:.2f}  D: {z:.2f}",
            position='upper_edge', font_size=16, color='black'
        )
        self.plotter.update()
...
class MultiSensorPADPipeline:
    def __init__(self, serial_port='/dev/ttyACM0', baudrate=115200,
        ...
        self.feature_extractor = MultimodalFeatureExtractor(fs_eeg, fs_ppg, fs_gsr, use_wavelet=False)
        ...
        rawdata_folder = "emodata"
        self.data_dir = os.path.join(script_dir, rawdata_folder)
        os.makedirs(self.data_dir, exist_ok=True)
        timestamp_str = datetime.now().strftime("%Y%m%d_%H%M%S")
        self.eeg_csv = open(os.path.join(self.data_dir, f"eeg_{timestamp_str}.csv"), "w")
        self.eeg_csv.write("timestamp,ch1,ch2\n")
        self.ppg_csv = open(os.path.join(self.data_dir, f"ppg_{timestamp_str}.csv"), "w")
        self.ppg_csv.write("timestamp,value\n")
        self.gsr_csv = open(os.path.join(self.data_dir, f"gsr_{timestamp_str}.csv"), "w")
        self.gsr_csv.write("timestamp,value\n")
        logger.info(f"原始数据将保存至: {self.data_dir}/eeg_{timestamp_str}.csv, {self.data_dir}/ppg_{timestamp_str}.csv, {self.data_dir}/gsr_{timestamp_str}.csv")

        self.feature_extract_lock = asyncio.Lock()
        self.feature_extract_busy = False

        # ===== ASR相关属性（使用asrpy）=====
        self.asr_calibration_duration = 30  # 秒
        self.asr_calib_needed = int(self.fs_eeg * self.asr_calibration_duration)  # 需要的点数
        self.asr_calib_ch1 = []   # 存放校准用ch1数据
        self.asr_calib_ch2 = []   # 存放校准用ch2数据
        self.asr_calibrated = False  # 是否已完成校准
        self.asr = None            # 训练好的ASR对象
        self.asr_cutoff = 20       # ASR cutoff参数，可根据需要调整
    ...
    async def find_and_connect_eeg(self):
        ...
        if not eeg_device:
            raise RuntimeError("未找到KSEEG102设备，且上次连接尝试失败")
    ...
    def _eeg_callback(self, timestamp, ch1, ch2):
        asyncio.run_coroutine_threadsafe(self.eeg_queue.put((timestamp, ch1, ch2)), asyncio.get_event_loop())
        logger.debug(f"EEG callback: ts={timestamp:.3f}, ch1={ch1:.2f}, ch2={ch2:.2f}")
    ...
    async def process_queues(self):
        ...
        while self.running:
            now = time.time()
            cutoff = now - 70
            while self.eeg_buffer and self.eeg_buffer[0][0] < cutoff:
                self.eeg_buffer.popleft()
            while self.ppg_buffer and self.ppg_buffer[0][0] < cutoff:
                self.ppg_buffer.popleft()
            while self.gsr_buffer and self.gsr_buffer[0][0] < cutoff:
                self.gsr_buffer.popleft()

            try:
                while not self.eeg_queue.empty():
                    ts, ch1, ch2 = self.eeg_queue.get_nowait()
                    self.eeg_buffer.append((ts, ch1, ch2))
                    self.eeg_csv.write(f"{ts},{ch1},{ch2}\n")
                    self.eeg_csv.flush()

                    # ===== ASRpy校准数据收集 =====
                    if ASR_AVAILABLE and not self.asr_calibrated:
                        self.asr_calib_ch1.append(ch1)
                        self.asr_calib_ch2.append(ch2)
                        if len(self.asr_calib_ch1) >= self.asr_calib_needed:
                            # 达到所需点数，进行校准
                            logger.info(f"已收集 {len(self.asr_calib_ch1)} 个EEG点，开始ASRpy校准...")
                            # 截取所需长度（取前N个）
                            ch1_arr = np.array(self.asr_calib_ch1[:self.asr_calib_needed])
                            ch2_arr = np.array(self.asr_calib_ch2[:self.asr_calib_needed])
                            data = np.vstack([ch1_arr, ch2_arr])  # shape (2, N)
                            try:
                                info = mne.create_info(ch_names=['ch1','ch2'], sfreq=self.fs_eeg, ch_types='eeg')
                                raw_calib = mne.io.RawArray(data, info)
                                
                                # 使用asrpy进行拟合
                                asr = asrpy.ASR(sfreq=self.fs_eeg, cutoff=self.asr_cutoff)
                                asr.fit(raw_calib)
                                
                                self.asr = asr
                                self.feature_extractor.set_asr(asr, cutoff=self.asr_cutoff)
                                logger.info(f"ASRpy校准成功 (cutoff={self.asr_cutoff})，已启用实时伪影去除")
                            except Exception as e:
                                logger.error(f"ASRpy校准失败: {e}")
                                import traceback
            ...
            try:
                while not self.serial_queue.empty():
                    ts, ppg, gsr = self.serial_queue.get_nowait()
                    self.ppg_buffer.append((ts, ppg))
                    self.gsr_buffer.append((ts, gsr))
                    self.ppg_csv.write(f"{ts},{ppg}\n")
                    self.gsr_csv.write(f"{ts},{gsr}\n")
                    self.ppg_csv.flush()
                    self.gsr_csv.flush()
            except queue.Empty:
                pass
            ...
            counter += 1
            if counter % 100 == 0:
                eeg_oldest = self.eeg_buffer[0][0] if self.eeg_buffer else 0
                eeg_newest = self.eeg_buffer[-1][0] if self.eeg_buffer else 0
                logger.info(f"process_queues alive, eeg_buffer size: {len(self.eeg_buffer)}, "
                            f"oldest={eeg_oldest:.2f}, newest={eeg_newest:.2f}, "
                            f"ppg_buffer size: {len(self.ppg_buffer)}, gsr_buffer size: {len(self.gsr_buffer)}")
            await asyncio.sleep(0.01)

    def _serial_thread_func(self):
        reader = SerialReader(self.serial_port, self.baudrate, self.serial_queue)
        reader.start()
        reader.join()
    ...
    async def _process_one_window(self):
        ...
        logger.info(f"窗口 {t_start_eeg:.2f}~{t_end:.2f} (EEG) 和 {t_start_ppg_gsr:.2f}~{t_end:.2f} (PPG/GSR) 数据量: "
                    f"eeg={len(eeg_data)}, ppg={len(ppg_data)}, gsr={len(gsr_data)}")
        ...
        for key, value in features.items():
            if np.isfinite(value):
                if key not in self.feature_stats:
                    self.feature_stats[key] = {'min': value, 'max': value}
                else:
                    self.feature_stats[key]['min'] = min(self.feature_stats[key]['min'], value)
                    self.feature_stats[key]['max'] = max(self.feature_stats[key]['max'], value)
        ...
        for name, px, ay, dz in self.base_emotions:
            dx = pad['P'] - px
            dy = pad['A'] - ay
            dz_ = pad['D'] - dz
        ...
        if self.visualizer:
            self.visualizer.update(pad)

        print(f"[{time.strftime('%H:%M:%S')}] P={pad['P']:.3f} A={pad['A']:.3f} D={pad['D']:.3f} | 靠近 {closest_emotion} 星球 (距离={distance:.3f})")

    async def process_windows(self):
        await asyncio.sleep(max(self.eeg_window_sec, self.ppg_gsr_window_sec))
        while self.running:
            if self.feature_extract_busy:
                logger.debug("特征提取繁忙，跳过当前窗口")
            else:
                async with self.feature_extract_lock:
                    self.feature_extract_busy = True
                    try:
    ...
    async def run(self):
        self.running = True
        self.visualizer = PADCubeVisualizer()

        self.ble_task = asyncio.create_task(self.run_ble())
        self.serial_thread = threading.Thread(target=self._serial_thread_func, daemon=True)
        self.serial_thread.start()
        queue_task = asyncio.create_task(self.process_queues())
        window_task = asyncio.create_task(self.process_windows())

        try:
            results = await asyncio.gather(self.ble_task, queue_task, window_task, return_exceptions=True)
            for i, res in enumerate(results):
                if isinstance(res, Exception):
                    logger.error(f"Task {i} exited with exception: {res}")
        except asyncio.CancelledError:
            pass
        finally:
            self.running = False
            ...
            if hasattr(self, 'eeg_csv') and self.eeg_csv:
                self.eeg_csv.close()
            if hasattr(self, 'ppg_csv') and self.ppg_csv:
                self.ppg_csv.close()
            if hasattr(self, 'gsr_csv') and self.gsr_csv:
                self.gsr_csv.close()
            print("\n" + "="*50)
            print("特征统计量范围（可用于校准缩放因子）:")
            print("="*50)
            for key in sorted(self.feature_stats.keys()):
                stats = self.feature_stats[key]
                print(f"{key}: min={stats['min']:.6f}, max={stats['max']:.6f}")
            print("程序已停止")
...
async def main():
    print("=" * 60)
    print("多模态情绪PAD实时计算系统 (ASRpy伪影去除版)")
    print("=" * 60)
    print("请确保:")
    print("  - KSEEG102 EEG设备已开机且可连接")
    print("  - 串口设备已连接（例如 /dev/ttyACM0）")
    print("  - 已安装所需Python库")
    print()

    SERIAL_PORT = '/dev/ttyACM0'   # Windows下如 'COM3'
    BAUDRATE = 115200

    pipeline = MultiSensorPADPipeline(
